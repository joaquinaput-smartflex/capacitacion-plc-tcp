...
**Estado general del proyecto:** 🟡 En desarrollo (Secciones 1 y 2 completadas, S1 ampliándose)
...
```
Sección 1 ██████████████░░░░░░  70%  🟡 Base lista, ampliando redes
...
## Sección 1: Repaso + Conexión (2.0 hs) ✅ COMPLETADA (material base)
...
| **PPT Redes ampliado (nuevo)** | ⬜ Pendiente | Contenido de redes extendido (ver detalle abajo) |

### Contenido nuevo a agregar (ampliar Redes_Fisico_a_Logico.pptx o crear PPT adicional):

**Direcciones IP y MAC:**
- [ ] Qué es una dirección MAC: identificador físico único de fábrica (ejemplo real del DVP12SE)
- [ ] Qué es una dirección IP: identificador lógico configurable
- [ ] Relación MAC ↔ IP: cómo se vinculan (tabla ARP), analogía DNI vs dirección postal
- [ ] Por qué NO puede haber IPs duplicadas en una red: conflictos, síntomas, cómo detectarlo
- [ ] Comando `arp -a` para ver la tabla MAC-IP de la PC

**DHCP vs IP estática:**
- [ ] Qué es DHCP: el router asigna IPs automáticamente (cómo funciona el lease)
- [ ] Qué es IP estática: el técnico asigna la IP manualmente (lo que hacemos con el PLC)
- [ ] Cuándo usar cada una: DHCP para PCs/celulares, IP estática para PLCs y dispositivos industriales
- [ ] IP pública vs IP privada: qué son los rangos privados (192.168.x.x, 10.x.x.x, 172.16-31.x.x)
- [ ] Por qué los PLCs siempre usan IP privada estática en redes industriales
- [ ] NAT: cómo una red privada sale a internet a través del router (conceptual)

**Herramientas de diagnóstico:**
- [ ] `ping`: qué hace, cómo interpretar la respuesta (TTL, tiempo, timeout)
- [ ] `ping -t` (continuo) y `ping -n 10` (cantidad específica)
- [ ] `tracert` (traceroute): para qué sirve, cómo leer los saltos, cuándo usarlo
- [ ] `ipconfig` / `ipconfig /all`: ver IP, máscara, gateway, MAC de la PC
- [ ] `arp -a`: ver tabla de asociación MAC-IP
- [ ] Práctica: usar cada comando en CMD y entender la salida

**Armar una red local:**
- [ ] Qué es un switch: dispositivo de capa 2, reenvía tramas por MAC, no asigna IP
- [ ] Qué es un router: dispositivo de capa 3, asigna IPs (DHCP), conecta redes distintas, tiene NAT
- [ ] Diferencia switch vs router: tabla comparativa clara con ejemplos
- [ ] Router WiFi doméstico = router + switch + access point (todo en uno)
- [ ] Topología estrella: todos los dispositivos al switch, el switch al router
- [ ] Ejemplo práctico: armar la red del taller (PLC + PCs + switch + router WiFi) con diagrama
- [ ] VLANs (conceptual): separar red de oficina de red industrial (mención, sin profundizar)
...
| Diagramas Ladder visuales (PPTX) | ✅ Listo | `Modulo2_Ladder_Modbus.pptx` |
...
Modulo2_Modbus_TCP.pptx              → Sección 2 (teoría + historia)
Modulo2_Ladder_Modbus.pptx           → Sección 2 (diagramas Ladder para instructor)
Guia_Practica_Modulo2_Modbus.docx    → Sección 2 (material para alumnos)
Cheatsheet_Modbus_TCP.pdf            → Sección 2 (referencia rápida 1 página)
...
2. ~~Sección 2: Modbus TCP completo~~ ✅ (PPT teoría + PPT Ladder + Guía + Cheat sheet)
3. **Sección 3: Socket TCP** ← próximo paso
