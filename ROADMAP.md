...
Sección 3 ████████████░░░░░░░░  55%  🟡 PPT listo
Sección 4 ████████████░░░░░░░░  55%  🟡 PPT listo
Sección 5 ████████████░░░░░░░░  55%  🟡 PPT listo
Sección 6 ████████████░░░░░░░░  55%  🟡 PPT listo
```
...
## Sección 2: Modbus TCP (3.0 hs) 🟡 AMPLIANDO

| Entregable | Estado | Archivo |
|------------|--------|---------|
| PPT Módulo 2 (3 hs, 27+slides) | 🟡 Ampliando | `Modulo2_Modbus_TCP.pptx` |
| Bloque A: Historia Modbus | ✅ Completo | Origen 1979, Modicon, timeline, por qué es popular |
| **Bloque A2: Redes en profundidad (NUEVO)** | ✅ Listo | IP/MAC, DHCP, ping, tracert, switch vs router |
...
Modulo2_Redes_Profundidad.pptx       → Sección 2 (IP, MAC, DHCP, ping, switch vs router)
...
### Contenido nuevo Bloque A2 – Redes en profundidad:

**Direcciones IP y MAC:**
- [ ] Qué es una dirección MAC: identificador físico único de fábrica (ejemplo real DVP12SE)
- [ ] Qué es una dirección IP: identificador lógico configurable
- [ ] Relación MAC ↔ IP: tabla ARP, analogía DNI vs dirección postal
- [ ] Por qué NO puede haber IPs duplicadas: conflictos, síntomas, detección
- [ ] Comando `arp -a` para ver la tabla MAC-IP

**DHCP vs IP estática:**
- [ ] DHCP: el router asigna IPs automáticamente (lease)
- [ ] IP estática: asignación manual (lo que hacemos con el PLC)
- [ ] Cuándo usar cada una: DHCP para PCs, estática para PLCs industriales
- [ ] IP pública vs IP privada: rangos privados (192.168.x.x, 10.x.x.x, 172.16-31.x.x)
- [ ] Por qué los PLCs siempre usan IP privada estática
- [ ] NAT: cómo una red privada sale a internet (conceptual)

**Herramientas de diagnóstico:**
- [ ] `ping`: interpretar respuesta (TTL, tiempo, timeout)
- [ ] `ping -t` (continuo) y `ping -n 10` (cantidad)
- [ ] `tracert`: para qué sirve, cómo leer los saltos
- [ ] `ipconfig /all`: ver IP, máscara, gateway, MAC de la PC
- [ ] `arp -a`: tabla de asociación MAC-IP
- [ ] Práctica en CMD con cada comando

**Armar una red local:**
- [ ] Switch: capa 2, reenvía por MAC, no asigna IP
- [ ] Router: capa 3, asigna IPs (DHCP), conecta redes, tiene NAT
- [ ] Diferencia switch vs router: tabla comparativa
- [ ] Router WiFi doméstico = router + switch + access point (3 en 1)
- [ ] Topología estrella: dispositivos → switch → router
- [ ] Ejemplo práctico: red del taller (PLC + PCs + switch + router WiFi)
- [ ] VLANs (conceptual): separar red oficina de red industrial
...
- [x] **Agregar Bloque A2 de redes al PPT (IP/MAC, DHCP, ping, tracert, switch/router)**
